...
public class DataSyncDao  extends BaseHibernateDao<DataSync, String> implements IDataSyncDao {
	...
	{
		String sql = "select * from " + tableName + " where create_date>'" + endUpdateTime + "' and modifed_date is null";
		SQLQuery query = getSession().createSQLQuery(sql);
	...
	{
		String sql = "select * from " + tableName + " where modifed_date>'" + endUpdateTime + "'";
		SQLQuery query = getSession().createSQLQuery(sql);
		query.addEntity(clazz);
		List<Object> list =  query.list();
		return list;
